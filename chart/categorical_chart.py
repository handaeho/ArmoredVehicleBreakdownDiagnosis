...
def day_charting(save_path, label, dataframe_2020, dataframe_2021):
    ...
    for group_name in groups_2020:
        ...
        # x = list(df_sub['day'])
        # y = list(df_sub[f'2020_{label_name}'])
        #
        # # 숫자 넣는 부분, height + 0.25로 숫자 약간 위로 위치 조정
        # for i in range(len(x)):
        #     height = y[i]
        #     plt.text(x[i], height + 0.1, '%.1f' % height, ha='center', va='bottom', size=8)

        # plotting
        plt.subplot(2, 1, 1)
        plt.title(f'2020 {label_name} Result for Day')
        plt.plot(df_sub['day'], df_sub[f'2020_{label_name}'])
        # plt.xticks(df_sub['day'])
    ...
    for group_name in groups_2021:
        ...
        # x = list(df_sub['day'])
        # y = list(df_sub[f'2021_{label_name}'])
        #
        # # 숫자 넣는 부분, height + 0.25로 숫자 약간 위로 위치 조정
        # for i in range(len(x)):
        #     height = y[i]
