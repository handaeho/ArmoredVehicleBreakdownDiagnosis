...
# 생성일자로 폴더 만들어서 저장 (생성일자 폴더가 없으면 생성, 있으면 저장)
...
class convert_data_csv:
    def __init__(self, year, dat_path, csv_path, parquet_path):
        self.year = year
    ...
    def find_path_periodic_file(self):
        all_file_path = []
        ...
        for (root, directories, files) in os.walk(self.dat_path):
        ...
        # print(len(periodic_file_path_list_all), periodic_file_path_list_all)

        return periodic_file_path_list_all

    def read_periodic_file_convert_csv(self, file_path):
        error_file = []
        ...
        try:
            periodic_data = pd.read_csv(file_path, sep='\t', skipinitialspace=True, encoding='CP949', skiprows=1, low_memory=False).drop(index=0)

            periodic_data.insert(loc=0, column='code', value=each_file_name)

            periodic_data.columns = ['FILENM', 'SDAID', 'OPERDATE', 'OPERTIME', 'DTTIME', 'TIME', 'ENGSTA', 'ENGSPD', 'ENGTOK', 'REQENDTOK',
            ...
            # 현재 날짜 폴더에 csv 파일 저장
            periodic_data.to_csv(self.csv_path + '/' + self.todayDate + '/periodic/' + str(each_file_name) + '.csv', sep=',', encoding='CP949', index=False)
    ...
    def find_path_event_file(self):
        ...
        # print(event_file_path_list_all)

        return event_file_path_list_all

    def read_event_file_convert_csv(self, file_path):
        error_file = []
        ...
        try:
            event_data = pd.read_csv(file_path, sep='\t', skipinitialspace=True, encoding='CP949', skiprows=1, low_memory=False).drop(columns='Unnamed: 8')

            event_data.insert(loc=0, column='code', value=each_file_name)

            event_data.columns = ['FILENM', 'SDAID', 'OPERDATE', 'OPERTIME', 'DATE', 'TIME', 'EVENTCD', 'MSG', 'STATE']
            ...
            # 현재 날짜 폴더에 csv 파일 저장
            event_data.to_csv(self.csv_path + '/' + self.todayDate + '/event/' + str(each_file_name) + '_Event' + '.csv', sep=',', encoding='CP949', index=False)
    ...
    def merge_periodic_csv_file(self):
        ...
        periodic_df_all.to_csv(self.csv_path + '/' + f'{self.todayDate}' + '/' + 'all_periodic_merged_data_with_code' + f'_{self.todayDate}' + '.csv', sep=',', encoding='CP949', index=False)

    def merge_event_csv_file(self):
        ...
        event_df_all.to_csv(self.csv_path + '/' + f'{self.todayDate}' + '/' + 'all_event_merged_data_with_code' + f'_{self.todayDate}' + '.csv', sep=',', encoding='CP949', index=False)

    # TODO: DB insert 메소드 만들기 (테이블 생성(이름: 차량 번호) 및 insert 까지)
...
if __name__ == '__main__':
    ...
    # 현재 날짜 폴더와 그 밑에 주기성, 비 주기성 폴더 생성
    createDirectory(csv_path + '/' + t.todayDate + '/periodic')
    createDirectory(csv_path + '/' + t.todayDate + '/event')
    createDirectory(parquet_path + '/' + t.todayDate)

    # 1. 주기성 데이터 -------------------------------------------------------
    print("START Periodic Data")

    # 1) 일자별 파일 각각 경로 탐색
    periodic_file_path_list = t.find_path_periodic_file()
    print(periodic_file_path_list)

    # 2) 각 경로에서 read 후, csv로 convert 후 저장
    periodic_error_list = []

    for periodic_file_path in tqdm(periodic_file_path_list):
        periodic_error = t.read_periodic_file_convert_csv(periodic_file_path)
        periodic_error_list.append(periodic_error)

    print(periodic_error_list)

    # 3) 각 csv 파일을 하나로 merge
    t.merge_periodic_csv_file()

    # 4) 하나로 합쳐진 csv 파일을 DB에 insert
    ...
    print(event_file_path_list)

    # 2) 각 경로에서 read 후, csv로 convert 후 저장
    event_error_list = []

    for event_file_path in tqdm(event_file_path_list):
        event_error = t.read_event_file_convert_csv(event_file_path)
        event_error_list.append(event_error)

    print(event_error_list)

    # 3) 각 csv 파일을 하나로 merge
    t.merge_event_csv_file()
